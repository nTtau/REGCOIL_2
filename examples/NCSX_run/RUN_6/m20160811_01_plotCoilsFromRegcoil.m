...
regcoilFilenames = {'NCSX_run/RUN_4/regcoil_out.regcoilNCSX.nc'};

%nescinFilenames = {'W7X/nescin.d23p4_tm_with_winding_surface_from_Drevlak'};
nescinFilenames = {'NCSX_run/RUN_4/nescin.out'};
...
for whichFile = 1:1
    ...
    contours = linspace(0,nfp,1+coilsPerHalfPeriod*numHalfPeriodsToPlot);
    contours(end)= [];
    dc = contours(2)-contours(1);
    contours = contours + 0.5*dc;
    fprintf(['CONTOUR SECTION ','\n'])
    ...
    check1 = coilsPerHalfPeriod*numHalfPeriodsToPlot
    fprintf(['Check val ',check1,'\n'])
    ...
    for j=1:coilsPerHalfPeriod*numHalfPeriodsToPlot
        ...
        TEST = numel(C)
        fprintf(['TEST ASSIGNED',TEST,'\n'])
        %fprintf(['Square brack test ',[this_contour,this_contour],'\n'])
        N = C(2,1);
        if N ~= size(C,2)-1
        ...
        fprintf(['Check size',size(contours_theta{j}),'\n']) 
    ...
    x = zeros(size(theta_2D+1));
    y = zeros(size(theta_2D+1));
    z = zeros(size(theta_2D+1));
    dxdtheta = zeros(size(theta_2D+1));
    dydtheta = zeros(size(theta_2D+1));
    dzdtheta = zeros(size(theta_2D+1));
    dxdzeta = zeros(size(theta_2D+1));
    dydzeta = zeros(size(theta_2D+1));
    dzdzeta = zeros(size(theta_2D+1));
   ...
   % fprintf("COORD TEST",'\n')
   % fprintf('%s',contours_x{:})

   % for i = 1:mnmax_nescin+1: 
   %     fprintf("X test:",x{i}{1})
    ...
    for j=1:coilsPerHalfPeriod*numHalfPeriodsToPlot
        plot3(coils_x{j},coils_y{j},coils_z{j},'r','LineWidth',2)
       % fprintf(['coords:','\n'])
       % fprintf(['x',contours_x,'\n'])        
    end
    ...
    for j=1:coilsPerHalfPeriod*numHalfPeriodsToPlot
        ...
        for k=1:4
            plot3(-coils_x{j}(:,k)+offset,-coils_y{j}(:,k),coils_z{j}(:,k),edgeColor,'LineWidth',0.1)
            
           
            xfile = "coords/x_" + j + "_" + k + ".csv"
            yfile = "coords/y_" + j + "_" + k +".csv"
            zfile = "coords/z_" + j + "_" + k + ".csv"
            fileIDx = fopen(xfile,'w');
            fprintf(fileIDx,'%s\n',-coils_x{j}(:,k))
            fileIDy = fopen(yfile,'w');
            fprintf(fileIDy,'%s\n',-coils_y{j}(:,k))
            fileIDz = fopen(zfile,'w');
            fprintf(fileIDz,'%s\n',coils_z{j}(:,k))
            fclose(fileIDx)
            fclose(fileIDy)
            fclose(fileIDz)
