...
regcoilFilenames = {'regcoil_out.regcoilNCSX.nc'};

%nescinFilenames = {'W7X/nescin.d23p4_tm_with_winding_surface_from_Drevlak'};
nescinFilenames = {'nescin.out'};
...
for whichFile = 1:1
    ...
    for j=1:coilsPerHalfPeriod*numHalfPeriodsToPlot
        ...
        for k=1:4
            ...
            centre = ["coords/c_" num2str(j) ".csv"]

            xfile = ["coords/x_" num2str(j) "_" num2str(k) ".csv"]
            yfile = ["coords/y_" num2str(j) "_" num2str(k) ".csv"]
            zfile = ["coords/z_" num2str(j) "_" num2str(k) ".csv"]
           
            cellsx = cellfun(@size,coils_x,'uni',false);
            max_count = (cellsx{j}(1,1))

            fileIDy = fopen(yfile,'w');
            fileIDx = fopen(xfile,'w');
            fileIDz = fopen(zfile,'w');
            fileIDc = fopen(centre,'w');

%contours_x{j}
            for l=1:max_count
                fprintf(fileIDx,'%s\n',num2str(coils_x{j}(l,k))+offset)
                fprintf(fileIDy,'%s\n',num2str(coils_y{j}(l,k)))
                fprintf(fileIDz,'%s\n',num2str(coils_z{j}(l,k)))
                fprintf(fileIDc,'%s\n',num2str(contours_x{j}(l,1)),num2str(contours_y{j}(l,1)),num2str(contours_z{j}(l,1)))
            end
