...
thickness= 0.1
print("Coil thickness:",thickness)
...
for j in range(numCoils):
   ...
   contour_R2.append(contour_zeta[j]*0)
   contour_Z2.append(contour_zeta[j]*0)

   contours_dxdtheta.append(contour_zeta[j]*0)
   contours_dydtheta.append(contour_zeta[j]*0)
   contours_dzdtheta.append(contour_zeta[j]*0)
   contours_dxdzeta.append(contour_zeta[j]*0)
   contours_dydzeta.append(contour_zeta[j]*0)
   contours_dzdzeta.append(contour_zeta[j]*0)
...
for imode in range(nmodes):
    ...
    crs = float(data[4])
    czc = float(data[5])
    print("crc,czs,crs,czc:",crc,czs,crs,czc)
    # Skip remaining columns
    for j in range(numCoils):
       angle = m*contour_theta[j] - n*contour_zeta[j]
       angle2 = contour_zeta[j]

       angle_new = m*contour_theta[j] - n*(contour_zeta[j] - thickness) 
       angle2_new = contour_zeta[j] + thickness

       contour_R[j] = contour_R[j] + crc*np.cos(angle) + crs*np.sin(angle)
       contour_Z[j] = contour_Z[j] + czs*np.sin(angle) + czc*np.cos(angle)

       contour_R2[j] = contour_R2[j] + crc*np.cos(angle_new) + crs*np.sin(angle_new)
       contour_Z2[j] = contour_Z2[j] + czs*np.sin(angle_new) + czc*np.cos(angle_new)

       contours_dxdtheta[j] = contours_dxdtheta[j] - m*crc*np.sin(angle)*np.cos(angle2)
       contours_dydtheta[j] = contours_dydtheta[j] - m*crc*np.sin(angle)*np.sin(angle2)
       contours_dzdtheta[j] = contours_dzdtheta[j] + m*czs*np.cos(angle)

            
       contours_dxdzeta[j] = contours_dxdzeta[j] - nfp*n*crc*np.sin(angle)*np.cos(angle2) - crc*np.cos(angle)*np.sin(angle2)
       contours_dydzeta[j] = contours_dydzeta[j] - nfp*n*crc*np.sin(angle)*np.sin(angle2) + crc*np.cos(angle)*np.cos(angle2)
       contours_dzdzeta[j] = contours_dzdzeta[j] + nfp*n*czs*np.cos(angle)
...
maxR=0
for j in range(numCoils):
   file_name= "test_" + str(j) +".csv"
   coilfile = open(file_name, "w")
   print("x,y,z",file=coilfile)
   ...
   contour_X2.append(contour_R2[j]*np.cos(contour_zeta[j]-thickness))
   contour_Y2.append(contour_R2[j]*np.sin(contour_zeta[j]-thickness))

   print("contour X size: ",len(contour_X[j]))
   points = len(contour_X[j]) 
   point_count = 0 
   while point_count <= points-1: 

      print(contour_X[j][point_count],",",contour_Y[j][point_count],",",contour_Z[j][point_count],file=coilfile)
...
for j in range(numCoils):
    ...
    coils_x= np.zeros((size1,4))
    coils_y= np.zeros((size1,4))
    coils_z= np.zeros((size1,4))
    # Compute normal direction:

        
    # Compute tangent direction:
    for indices in range(0,(len(contour_X[j]))):

        if indices ==0: 

            prev_index = (len(contour_X[j])-1)

        else: 
            prev_index = indices-1

        if indices ==(len(contour_X[j])-1): 
        ...
        print("indices") 
        print(prev_index,indices,next_index)
        ...
        Tx[indices] = contour_X[j][next_index] - contour_X[j][prev_index]
        Ty[indices] = contour_Y[j][next_index] - contour_Y[j][prev_index]
        Tz[indices] = contour_Z[j][next_index] - contour_Z[j][prev_index]
    ...
    coils_x[:,0] = contour_X[j][:]
    coils_x[:,1] = contour_X2[j][:] #+ coil_thickness*(Nx[:]-Bx[:])
    coils_x[:,2] = contour_X[j][:] #+ coil_thickness*(-Nx[:]-Bx[:])
    coils_x[:,3] = contour_X[j][:] #+ coil_thickness*(-Nx[:]+Bx[:])

    coils_y[:,0] = contour_Y[j][:]
    coils_y[:,1] = contour_Y2[j][:]# + coil_thickness*(Ny[:]-By[:])
    coils_y[:,2] = contour_Y[j][:] #+ coil_thickness*(-Ny[:]-By[:])
    coils_y[:,3] = contour_Y[j][:] #+ coil_thickness*(-Ny[:]+By[:])

    coils_z[:,0] = contour_Z[j][:] 
    coils_z[:,1] = contour_Z2[j][:]# + coil_thickness*(Nz[:]-Bz[:])
    coils_z[:,2] = contour_Z[j][:] #+ coil_thickness*(-Nz[:]-Bz[:])
    coils_z[:,3] = contour_Z[j][:] #+ coil_thickness*(-Nz[:]+Bz[:])


    for loop_num in range(1,5):
        xfilename = "points/x_" + str(j+1) +"_" +str(loop_num) +".csv"
        xcoil = open(xfilename, "w")
        yfilename = "points/y_" + str(j+1) +"_" +str(loop_num) +".csv"
        ycoil = open(yfilename, "w")
        zfilename = "points/z_" + str(j+1) +"_" +str(loop_num) +".csv"
        zcoil = open(zfilename, "w")
        totfilename = "points/tot_" + str(j+1) +"_" +str(loop_num) +".csv"
        totcoil = open(totfilename, "w")



        for point_num in range(0,size1):

            print(coils_x[point_num,loop_num-1],file=xcoil)
            print(coils_y[point_num,loop_num-1],file=ycoil)
            print(coils_z[point_num,loop_num-1],file=zcoil)
            print(coils_x[point_num,loop_num-1],",",coils_y[point_num,loop_num-1],",",coils_z[point_num,loop_num-1],file=totcoil)
    
#            contour_X[j] + coil_thickness*(Nx-Bx), ...
#            contour_X[j] + coil_thickness*(-Nx-Bx), ...
#            contour_X[j] + coil_thickness*(-Nx+Bx), ...
        
#        coils_y{j} = [...
 #           contour_Y[j] + coil_thickness*(Ny+By), ...
  #          contour_Y[j] + coil_thickness*(Ny-By), ...
   #         contour_Y[j] + coil_thickness*(-Ny-By), ...
    #        contour_Y[j] + coil_thickness*(-Ny+By), ...
     #       contour_Y[j] + coil_thickness*(Ny+By)];
...
#        coils_z{j} = [...
 #           contour_Z[j] + coil_thickness*(Nz+Bz), ...
  #          contour_Z[j] + coil_thickness*(Nz-Bz), ...
   #         contour_Z[j] + coil_thickness*(-Nz-Bz), ...
    #        contour_Z[j] + coil_thickness*(-Nz+Bz), ...
     #       contour_Z[j] + coil_thickness*(Nz+Bz)];
...
# Write coils file
print("Coil filename:",coilsFilename)
